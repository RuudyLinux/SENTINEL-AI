...
export default function ErrorState({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center border border-critical/30 bg-critical/5 rounded-lg gap-2">
      ...
      {onRetry && (
        <button onClick={onRetry} className="mt-1 text-xs border border-border rounded px-3 py-1.5 hover:border-accent">
          RETRY
        </button>
      )}
    </div>
